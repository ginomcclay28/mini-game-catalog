...
(function () {
  ...
  /* แปลงพิกัดภายในฝั่ง -> พิกัดจอ (ใช้ยิงอนุภาค ซึ่งวาดในพิกัดจอ) */
  function toScr(a, i, lx, ly) {
    var s = side(a, i);
    return s.flip ? { x: s.x + s.w - lx, y: s.y + s.h - ly } : { x: s.x + lx, y: s.y + ly };
  }
  function pcol(i) { return i ? P2 : P1; }
  ...
  R('vstug', {
    ...
    timeout: function (a) { var d = a.data; if (!d.over) d.over = vsEnd(a, d.s[0], d.s[1]); },
    update: function (dt, a) {
      var d = a.data;
      d.pos *= Math.pow(.6, dt);
      if (!d.over && Math.abs(d.pos) > 1) { d.over = d.pos > 0 ? 1 : 2; a.beep(1200, .35, 'triangle'); a.flash(pcol(d.over - 1), .3); a.shake(.027, .35); a.end(win(a, d.over), d.s[0] + ' : ' + d.s[1]); }
    },
    down: function (x, y, a) {
      var d = a.data, w = which(a, x, y); if (w < 0 || d.over) return;
      d.s[w]++; d.pos += (w === 0 ? -.045 : .045); a.beep(400 + d.s[w] * 3, .04, 'triangle');
      var p = toScr(a, w, side(a, w).w / 2, side(a, w).h * .55);
      a.puff(p.x, p.y, { n: 3, col: 'rgba(255,255,255,.6)', spread: 6.28, spd: a.mn * .3, size: a.mn * .008, life: .3, grav: 0 });
    },
    draw: function (g, a) {
      a.bg('#1a1040', '#3a1060');
      var d = a.data;
      [0, 1].forEach(function (i) {
        inSide(g, a, i, function (w, h) {
          ...
          g.fillRect(0, 0, w, h);
          /* แนวตั้ง: คะแนนชิดซ้าย ไม่ให้เชือกทับ */
          a.text('P' + (i + 1) + '  ' + d.s[i], a.port ? w * .2 : w / 2, h * .18, a.mn * .06, i ? P2 : P1);
          a.text(a.txt({ th: 'แตะรัว!', en: 'TAP!' }), a.port ? w * .8 : w / 2, h * .78, a.mn * .05, 'rgba(255,255,255,.55)');
        });
      });
      divider(g, a);
  ...
  /* จบเกมพร้อมคะแนนทั้งสองฝั่ง คืนค่า 1/2 = ผู้ชนะ, 3 = เสมอ */
  function vsEnd(a, s0, s1) {
    var w = s0 === s1 ? 3 : (s0 > s1 ? 1 : 2);
    a.end(w === 3 ? a.txt({ th: 'เสมอ!', en: 'DRAW!' }) : win(a, w), s0 + ' : ' + s1);
    return w;
  }

  /* ---------- 92 ปิงปอง 2 คน ---------- */
  var PADK = 1.753, PADO = .184;   /* ภาพไม้: ความสูงเทียบความยาวหน้าไม้, จุดกึ่งกลางหน้าไม้เยื้องจากกลางภาพ (วัดทีหลัง) */
  function drawPaddle(a, i, x, y, len, rot) {
    /* x,y = กึ่งกลางหน้าไม้  rot = ทิศด้าม (0 = ด้ามชี้ลง) */
    var sz = len * PADK, ox = Math.sin(rot) * sz * PADO, oy = -Math.cos(rot) * sz * PADO;
    return a.sprTint('paddle', pcol(i), null, x - ox, y - oy, sz, { rot: rot });
  }
  R('vspong', {
    time: 0, noScore: true,
    setup: function (a) {
      a.data.LO = { pw: a.port ? a.mn * .26 : a.mn * .03, ph: a.port ? a.mn * .03 : a.mn * .26, r: a.mn * .028, sp: a.mn * .62 };
      /* มีภาพไม้กลม: ให้ไม้ทั้งอันอยู่ในจอ ความหนาของเขตชน = ขนาดหน้าไม้ ลูกจะเด้งที่ขอบหน้าไม้ */
      var art = a.hasSpr('paddle');
      if (art) { if (a.port) a.data.LO.ph = a.data.LO.pw * .85; else a.data.LO.pw = a.data.LO.ph * .85; }
      /* ระนาบชน: มีภาพไม้ = กลางหน้าไม้ (ลูกกระดอนจากกลางไม้)  ไม่มีภาพ = ขอบแท่ง */
      a.data.LO.ct = (a.port ? a.data.LO.ph : a.data.LO.pw) * (art ? .5 : 1);
    ...
    update: function (dt, a) {
      ...
      if (a.port) {
        ...
        if (d.vy < 0 && d.y < top + L.ct) { if (Math.abs(d.x - d.p[1]) < L.pw / 2 + L.r) { d.vy = Math.abs(d.vy) * 1.04; d.vx += (d.x - d.p[1]) * 2; d.y = top + L.ct + L.r; a.beep(700, .05); hitfx(a, d.x, d.y); } }
        if (d.vy > 0 && d.y > bot - L.ct) { if (Math.abs(d.x - d.p[0]) < L.pw / 2 + L.r) { d.vy = -Math.abs(d.vy) * 1.04; d.vx += (d.x - d.p[0]) * 2; d.y = bot - L.ct - L.r; a.beep(700, .05); hitfx(a, d.x, d.y); } }
        ...
        if (d.vx < 0 && d.x < lft + L.ct) { if (Math.abs(d.y - d.p[0]) < L.ph / 2 + L.r) { d.vx = Math.abs(d.vx) * 1.04; d.vy += (d.y - d.p[0]) * 2; d.x = lft + L.ct + L.r; a.beep(700, .05); hitfx(a, d.x, d.y); } }
        if (d.vx > 0 && d.x > rgt - L.ct) { if (Math.abs(d.y - d.p[1]) < L.ph / 2 + L.r) { d.vx = -Math.abs(d.vx) * 1.04; d.vy += (d.y - d.p[1]) * 2; d.x = rgt - L.ct - L.r; a.beep(700, .05); hitfx(a, d.x, d.y); } }
    ...
    draw: function (g, a) {
      ...
      divider(g, a);
      if (a.port) {
        if (!drawPaddle(a, 1, d.p[1], a.mn * .06 + L.ph / 2, L.pw, Math.PI + .6)) a.fillRR(d.p[1] - L.pw / 2, a.mn * .06 - L.ph, L.pw, L.ph, L.ph / 2, P2);
        if (!drawPaddle(a, 0, d.p[0], a.H - a.mn * .06 - L.ph / 2, L.pw, .6)) a.fillRR(d.p[0] - L.pw / 2, a.H - a.mn * .06, L.pw, L.ph, L.ph / 2, P1);
        a.text(d.s[1] + '', a.W * .12, a.H * .30, a.mn * .1, 'rgba(255,46,136,.5)');
        a.text(d.s[0] + '', a.W * .12, a.H * .70, a.mn * .1, 'rgba(0,212,255,.5)');
      } else {
        if (!drawPaddle(a, 0, a.mn * .06 + L.pw / 2, d.p[0], L.ph, Math.PI / 2 - .6)) a.fillRR(a.mn * .06 - L.pw, d.p[0] - L.ph / 2, L.pw, L.ph, L.pw / 2, P1);
        if (!drawPaddle(a, 1, a.W - a.mn * .06 - L.pw / 2, d.p[1], L.ph, -Math.PI / 2 + .6)) a.fillRR(a.W - a.mn * .06, d.p[1] - L.ph / 2, L.pw, L.ph, L.pw / 2, P2);
        a.text(d.s[0] + '', a.W * .30, a.H * .16, a.mn * .1, 'rgba(0,212,255,.5)');
        a.text(d.s[1] + '', a.W * .70, a.H * .16, a.mn * .1, 'rgba(255,46,136,.5)');
      }
      if (!a.spr('ball', null, d.x, d.y, L.r * 2.2, { rot: a.now * 6 })) a.circle(d.x, d.y, L.r, '#fff');
      if (d.over) banner(a, g, win(a, d.over), d.over === 2 ? P2 : P1);
      a.head(a.txt({ th: 'ใครถึง 7 แต้มก่อนชนะ', en: 'First to 7 points' }));
    }
  });
  function hitfx(a, x, y) { a.puff(x, y, { n: 6, col: '#fff', spread: 6.28, spd: a.mn * .3, size: a.mn * .007, life: .3, grav: 0 }); }
  function pad(a, x, y) {
  ...
  function point(a, p) {
    var d = a.data;
    d.s[p]++; a.beep(p ? 400 : 900, .2); a.flash(pcol(p), .2); a.shake(.010, .15);
    if (d.s[p] >= 7) { d.over = p + 1; a.beep(1200, .35, 'triangle'); a.end(win(a, p + 1), d.s[0] + ' : ' + d.s[1]); }
    else serve(a, p);
  }
  ...
  R('vsair', {
    ...
    update: function (dt, a) {
      ...
      d.m.forEach(function (m) {
        /* ความเร็วของที่ตี (จากการลาก) เอาไปเพิ่มแรงกระแทก: ง้างไกล-ตีเร็ว = ลูกพุ่งแรง */
        var mvx = (m.x - (m.lx === undefined ? m.x : m.lx)) / dt, mvy = (m.y - (m.ly === undefined ? m.y : m.ly)) / dt;
        m.lx = m.x; m.ly = m.y;
        var dx = d.px - m.x, dy = d.py - m.y, len = Math.hypot(dx, dy) || 1;
        if (len < L.mr + L.pr) {
          var msp = Math.hypot(mvx, mvy), toward = (mvx * dx + mvy * dy) / len;   /* เฉพาะส่วนที่ตีเข้าหาลูก */
          var sp = Math.max(a.mn * .55, Math.hypot(d.pvx, d.pvy) * .9) + Math.max(0, toward) * 1.3;
          sp = Math.min(a.mn * 2.6, sp);
          d.pvx = dx / len * sp; d.pvy = dy / len * sp;
          if (msp > a.mn * .8) { a.shake(.006 + Math.min(.012, msp / a.mn * .004), .12); a.puff(d.px, d.py, { n: 8, col: '#fff', spread: 6.28, spd: a.mn * .4, size: a.mn * .008, life: .3, grav: 0 }); }
          d.px = m.x + dx / len * (L.mr + L.pr); d.py = m.y + dy / len * (L.mr + L.pr);
          a.beep(620, .05); hitfx(a, d.px, d.py);
        }
      });
    },
    ...
    draw: function (g, a) {
      ...
      if (!a.sprTint('mallet', P1, null, d.m[0].x, d.m[0].y, L.mr * 2.1)) a.circle(d.m[0].x, d.m[0].y, L.mr, P1);
      if (!a.sprTint('mallet', P2, null, d.m[1].x, d.m[1].y, L.mr * 2.1)) a.circle(d.m[1].x, d.m[1].y, L.mr, P2);
      if (!a.spr('puck', null, d.px, d.py, L.pr * 2.1)) a.circle(d.px, d.py, L.pr, '#fff');
  ...
  function goal(a, p) {
    var d = a.data;
    d.s[p]++; a.beep(p ? 400 : 950, .25, 'triangle'); a.flash(pcol(p), .22); a.shake(.013, .18);
    a.puff(d.px, d.py, { n: 14, col: pcol(p), spread: 6.28, spd: a.mn * .5, size: a.mn * .01, life: .5, grav: 0 });
    if (d.s[p] >= 5) { d.over = p + 1; a.end(win(a, p + 1), d.s[0] + ' : ' + d.s[1]); } else puck(a);
  }

  /* ---------- 94 กดเร็วแข่งกัน ---------- */
  R('vstap', {
    noScore: true,
    setup: function (a) { a.data.s = [0, 0]; a.data.go = 0; a.data.cd = 3; a.data.pr = [0, 0]; a.data.rings = []; a.data.heat = [0, 0]; a.data.pops = []; },
    update: function (dt, a) {
      var d = a.data;
      d.pr[0] -= dt; d.pr[1] -= dt;
      d.heat[0] = Math.max(0, d.heat[0] - dt * 1.5); d.heat[1] = Math.max(0, d.heat[1] - dt * 1.5);   /* ความร้อนแรงของแต่ละฝั่ง ลดลงเอง */
      d.rings.forEach(function (r) { r.t += dt; }); d.rings = d.rings.filter(function (r) { return r.t < .45; });
      d.pops.forEach(function (o) { o.t += dt; }); d.pops = d.pops.filter(function (o) { return o.t < .6; });
      if (!d.go) { d.cd -= dt; if (d.cd <= 0) { d.go = 1; a.beep(1000, .2, 'triangle'); a.flash('#fff', .15); } a.addTime(dt); }
    },
    timeout: function (a) { var d = a.data; if (!d.over) d.over = vsEnd(a, d.s[0], d.s[1]); },
    down: function (x, y, a) {
      var d = a.data, w = which(a, x, y); if (w < 0 || !d.go) return;
      d.s[w]++; a.beep(420 + d.s[w] * 4, .035, 'triangle'); d.pr[w] = .09;
      d.heat[w] = Math.min(1, d.heat[w] + .25);
      var sd = side(a, w), p = toScr(a, w, sd.w / 2, sd.h * .5);
      a.puff(p.x, p.y, { n: 8, col: pcol(w), spread: 6.28, spd: a.mn * .7, size: a.mn * .01, life: .35, grav: 0 });
      d.rings.push({ w: w, t: 0 });
      d.pops.push({ w: w, t: 0, x: a.rnd(-.25, .25), y: 0 });
      a.shake(.003, .06);
    },
    draw: function (g, a) {
      a.bg('#2a0d4a', '#c8461e');
      ...
      [0, 1].forEach(function (i) {
        inSide(g, a, i, function (w, h) {
          /* พื้นหลังเต้นตามจังหวะ: ยิ่งกดรัวยิ่งแว้บแรง + แถบแสงหมุน */
          var beat = d.go ? (Math.sin(a.now * 9) * .5 + .5) : 0, heat = d.heat[i];
          var al = .14 + beat * .08 + heat * .3;
          g.fillStyle = i ? 'rgba(255,46,136,' + al + ')' : 'rgba(0,212,255,' + al + ')';
          g.fillRect(0, 0, w, h);
          if (d.go) {
            g.save(); g.translate(w / 2, h / 2); g.rotate(a.now * (.6 + heat * 2)); g.fillStyle = 'rgba(255,255,255,' + (.05 + heat * .1) + ')';
            for (var k = 0; k < 8; k++) { g.rotate(Math.PI / 4); g.beginPath(); g.moveTo(0, 0); g.lineTo(w * 1.2, -h * .12); g.lineTo(w * 1.2, h * .12); g.closePath(); g.fill(); }
            g.restore();
            if (heat > .5) { g.strokeStyle = 'rgba(255,255,255,' + ((heat - .5) * (.3 + beat * .5)) + ')'; g.lineWidth = a.mn * .02; g.strokeRect(a.mn * .01, a.mn * .01, w - a.mn * .02, h - a.mn * .02); }
          }
          var r = Math.min(w, h) * .22, dn = d.pr[i] > 0;
          /* คลื่นกระแทกจากปุ่ม */
          d.rings.forEach(function (rg) {
            if (rg.w !== i) return;
            var f = rg.t / .45;
            g.strokeStyle = 'rgba(255,255,255,' + (1 - f) * .8 + ')'; g.lineWidth = a.mn * .012 * (1 - f) + 1;
            g.beginPath(); g.arc(w / 2, h * .5, r * (1.1 + f * 1.6), 0, 6.29); g.stroke();
          });
          var sq = dn ? 1.08 : 1;
          if (!a.sprTint(dn ? 'btn-down' : 'btn', pcol(i), null, w / 2, h * .5, r * 2.4, { sx: sq, sy: 2 - sq })) a.circle(w / 2, h * .5, r * (dn ? .95 : 1), pcol(i));
          a.text(d.s[i] + '', w / 2, h * .5 - r * (a.hasSpr('btn') ? .15 : 0), r * .7 * (dn ? 1.12 : 1), '#fff');
          d.pops.forEach(function (o) {
            if (o.w !== i) return;
            var f = o.t / .6;
            g.globalAlpha = 1 - f; a.text('+1', w / 2 + o.x * w, h * .5 - r * 1.3 - f * h * .2, a.mn * .05 * (1 + f * .3), '#fff'); g.globalAlpha = 1;
          });
  ...
  R('vsreact', {
    ...
    update: function (dt, a) {
      ...
      if (d.over) return;
      d.t -= dt;
      if (d.st === 'wait' && d.t <= 0) { d.st = 'go'; a.beep(1000, .12); a.flash('#fff', .12); }
      else if (d.st === 'res' && d.t <= 0) {
        if (d.s[0] >= 5 || d.s[1] >= 5) { d.over = vsEnd(a, d.s[0], d.s[1]); return; }
    ...
    down: function (x, y, a) {
      ...
      if (d.st === 'wait') {
        ...
        a.beep(160, .3, 'square'); d.st = 'res'; d.t = 1.3; a.shake(.020, .25);
      } else if (d.st === 'go') {
        d.s[w]++; d.msg = a.txt({ th: 'P' + (w + 1) + ' เร็วกว่า!', en: 'P' + (w + 1) + ' was faster!' });
        a.beep(1050, .2, 'triangle'); d.st = 'res'; d.t = 1.3; a.flash(pcol(w), .25);
        a.puff(a.W / 2, a.H / 2, { n: 18, col: pcol(w), spread: 6.28, spd: a.mn * .6, size: a.mn * .011, life: .5, grav: 0 });
      }
    },
    draw: function (g, a) {
      var d = a.data;
      ...
      if (art) g.globalAlpha = .6;
      g.fillRect(0, 0, a.W, a.H); g.globalAlpha = 1;
      if (a.hasSpr('btn')) a.spr('btn', null, a.W / 2, a.H / 2, a.mn * .3 * (d.st === 'go' ? 1 + Math.sin(a.now * 20) * .04 : 1));
      [0, 1].forEach(function (i) {
        inSide(g, a, i, function (w, h) {
  ...
  R('vssumo', {
    ...
    update: function (dt, a) {
      ...
      if (Math.abs(d.pos) > 1) {
        ...
        a.beep(1000, .25, 'triangle'); d.t = 1.4; a.shake(.023, .3); a.flash(pcol(w), .25);
        if (d.round[w] >= 3) { d.over = w + 1; a.end(win(a, w + 1), d.round[0] + ' : ' + d.round[1]); }
      }
    },
    down: function (x, y, a) {
      var d = a.data, w = which(a, x, y); if (w < 0 || d.t > 0 || d.over) return;
      d.pos += (w === 0 ? .05 : -.05); a.beep(380, .04, 'square');
      a.puff(a.W / 2 + (a.port ? 0 : d.pos * a.mn * .3), a.H / 2 + (a.port ? -d.pos * a.mn * .3 : 0) + a.mn * .06,
        { n: 3, col: 'rgba(255,230,190,.7)', spread: 6.28, spd: a.mn * .25, size: a.mn * .009, life: .35, grav: 0 });
    },
    draw: function (g, a) {
      a.bg('#3a1a0a', '#c8761e');
      var d = a.data, cx = a.W / 2, cy = a.H / 2, Rr = Math.min(a.W, a.H) * .38;
      if (!a.spr('ring', null, cx, cy, Rr * 2)) { a.circle(cx, cy, Rr, '#e3c08a'); a.circle(cx, cy, Rr * .92, '#c99a5b'); }
      var off = d.pos * Rr * .8;
      ...
      var pr = a.port ? -Math.PI / 2 : 0, wob = Math.sin(a.now * 10) * .05;
      a.spr('p1', '🐯', ax, ay, Rr * .42, { rot: pr + wob });
      a.spr('p2', '🐻', bx, by, Rr * .42, { rot: pr - wob });
      [0, 1].forEach(function (i) {
        inSide(g, a, i, function (w, h) {
  ...
  R('vscatch', {
    ...
    setup: function (a) {
      ...
      a.data.ITEMS = [{ k: 'apple', e: '🍎' }, { k: 'star', e: '⭐' }, { k: 'gift', e: '🎁' }, { k: 'candy', e: '🍬' }];
    ...
    update: function (dt, a) {
      ...
      [0, 1].forEach(function (i) {
        ...
        if (d.sp[i] <= 0) {
          ...
          var it = bomb ? { k: 'bomb', e: '💣' } : a.pick(d.ITEMS);
          d.it[i].push({ x: a.rnd(L.is, s.w - L.is), y: -L.is, v: a.rnd(a.mn * .32, a.mn * .5), b: bomb, e: it.e, k: it.k, r: a.rnd(-.4, .4) });
          d.sp[i] = a.rnd(.45, .8);
        }
        for (var k = d.it[i].length - 1; k >= 0; k--) {
          ...
          if (o.y > s.h - L.is * 1.2 && Math.abs(o.x - d.b[i]) < L.hw) {
            d.it[i].splice(k, 1);
            var p = toScr(a, i, o.x, o.y);
            if (o.b) { d.s[i] = Math.max(0, d.s[i] - 15); a.beep(150, .2, 'square'); a.shake(.017, .2); a.puff(p.x, p.y, { n: 12, col: '#ff8a3d', spread: 6.28, spd: a.mn * .5, size: a.mn * .01, life: .45 }); }
            else { d.s[i] += 10; a.beep(880, .06); a.puff(p.x, p.y, { n: 6, col: pcol(i), spread: 6.28, spd: a.mn * .35, size: a.mn * .008, life: .35, grav: 0 }); }
    ...
    draw: function (g, a) {
      ...
      [0, 1].forEach(function (i) {
        inSide(g, a, i, function (w, h) {
          ...
          g.fillRect(0, 0, w, h);
          d.it[i].forEach(function (o) { a.spr(o.k, o.e, o.x, o.y, L.is * 1.15, { rot: o.r }); });
          var bi = a.sprImg('basket');
          if (bi) {
            var bh = L.hw * 2.2 * bi.height / bi.width;
            a.sprTint('basket', pcol(i), null, d.b[i], h - L.is * .75, bh);
          } else {
            g.beginPath();
  ...
  R('vsmemory', {
    time: 0, noScore: true,
    setup: function (a) {
      var faces = [{ k: 'apple', e: '🍎' }, { k: 'orange', e: '🍊' }, { k: 'grape', e: '🍇' }, { k: 'strawberry', e: '🍓' }, { k: 'kiwi', e: '🥝' }, { k: 'pineapple', e: '🍍' }];
    ...
    update: function (dt, a) {
      ...
      if (d.open[0].f.k === d.open[1].f.k) {
        d.open[0].st = 2; d.open[1].st = 2; d.s[d.turn]++; d.left--; a.beep(950, .12); a.flash(pcol(d.turn), .2);
        d.open.forEach(function (c) {
          var L = d.LO, bx = L.ox + (c.col + .45) * L.cell, by = L.oy + (c.r + .45) * L.cell;
          a.puff(bx, by, { n: 10, col: pcol(d.turn), spread: 6.28, spd: a.mn * .4, size: a.mn * .009, life: .45, grav: 0 });
        });
        if (!d.left) d.over = vsEnd(a, d.s[0], d.s[1]);
      } else { d.open[0].st = 0; d.open[1].st = 0; d.turn = 1 - d.turn; a.beep(220, .12, 'square'); a.shake(.007, .12); }
      d.open = [];
    },
    down: function (x, y, a) {
    ...
    draw: function (g, a) {
      ...
      d.c.forEach(function (c) {
        var bx = L.ox + c.col * L.cell, by = L.oy + c.r * L.cell, s = L.cell * .9;
        if (c.st === 0) {
          if (!a.sprTint('back', d.turn ? P2 : P1, null, bx + s / 2, by + s / 2, s)) { a.fillRR(bx, by, s, s, s * .12, d.turn ? P2 : P1); a.text('?', bx + s / 2, by + s / 2, s * .4, 'rgba(255,255,255,.85)'); }
        } else {
          var bi = a.sprImg('back'), cw = bi ? s * bi.width / bi.height : s;
          a.fillRR(bx + s / 2 - cw / 2, by, cw, s, s * .1, c.st === 2 ? '#2fe08a' : '#fff');
          a.spr(c.f.k, c.f.e, bx + s / 2, by + s / 2, s * .6);
        }
      });
      [0, 1].forEach(function (i) {
        inSide(g, a, i, function (w, h) {
  ...
  R('vsshoot', {
    ...
    down: function (x, y, a) {
      ...
      var sp = toScr(a, w, tx, ty);
      if (p) {
        d.s[w] += p; d.fx[w] = .3; a.beep(p === 50 ? 1150 : 800, .1);
        a.puff(sp.x, sp.y, { n: p === 50 ? 16 : 8, col: p === 50 ? '#ffd23f' : pcol(w), spread: 6.28, spd: a.mn * .45, size: a.mn * .009, life: .4, grav: 0 });
        if (p === 50) a.flash(pcol(w), .18);
      }
      else { a.beep(200, .12, 'square'); a.shake(.007, .1); }
    },
    draw: function (g, a) {
      a.bg('#123b57', '#0d6b6b');
      var d = a.data, L = d.LO;
      ...
      [0, 1].forEach(function (i) {
        inSide(g, a, i, function (w, h) {
          ...
          if (!a.spr('target', null, tx, ty, L.r * 2.1)) for (var k = 0; k < 4; k++) a.circle(tx, ty, L.r * rad[k], cols[k]);
          if (d.fx[i] > .15) a.spr('burst', null, tx, ty, L.r * 2.6 * (1 + (.3 - d.fx[i]) * 3), { alpha: (d.fx[i] - .15) / .15 });
  ...
  R('vsrun', {
    ...
    update: function (dt, a) {
      ...
      if (!d.over) [0, 1].forEach(function (i) {
        if (d.p[i] >= d.goal) { d.over = i + 1; a.beep(1250, .35, 'triangle'); a.flash(pcol(i), .3); a.shake(.020, .3); a.end(win(a, i + 1), Math.floor(d.p[0]) + ' : ' + Math.floor(d.p[1])); }
      });
    },
    down: function (x, y, a) {
      var d = a.data, w = which(a, x, y); if (w < 0 || d.over) return;
      ...
      if (half === d.nx[w]) {
        d.p[w] += 2.2; d.nx[w] = 1 - half; a.beep(500 + d.p[w] * 2, .04, 'triangle');
        var px = lo.w * .08 + (lo.w * .82) * Math.min(1, d.p[w] / d.goal), sp = toScr(a, w, px - lo.w * .04, lo.h * .6);
        a.puff(sp.x, sp.y, { n: 3, col: 'rgba(255,230,200,.6)', spread: 6.28, spd: a.mn * .2, size: a.mn * .009, life: .35, grav: -a.mn * .2 });
      }
      else { d.p[w] = Math.max(0, d.p[w] - .8); a.beep(200, .05, 'square'); a.shake(.005, .1); }
    },
    draw: function (g, a) {
      a.bg('#3a1060', '#c8761e');
      var d = a.data;
      [0, 1].forEach(function (i) {
        inSide(g, a, i, function (w, h) {
          ...
          var bob = d.p[i] > 0 ? Math.abs(Math.sin(a.now * 14 + i * 2)) * h * .03 : 0;
          a.spr(i ? 'p2' : 'p1', '🏃', px, h * .51 - bob, Math.min(w, h) * (a.hasSpr('p1') ? .24 : .16));
